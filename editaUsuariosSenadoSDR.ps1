<#
Programa: editaUsuariosSenadoSDR
Objetivo: Edição de usuários para uso no SDR
...
$PathLog = "C:\Users\p_7029\ownCloud\_trabalho\teletrabalho\sdr\editaUsuariosSenadoSDR-" + (Get-Date -UFormat %Y-%m-%d-%H-%M) + ".log"
$PathLogErro = "C:\Users\p_7029\ownCloud\_trabalho\teletrabalho\sdr\editaUsuariosSenadoSDR-" + (Get-Date -UFormat %Y-%m-%d-%H-%M) + ".logerro"

# Caminho e nome do arquivo CSV com a lista de usuarios a ser importada
$PathLista = "C:\Users\p_7029\ownCloud\_trabalho\teletrabalho\sdr\usuariosSenadoSDR.csv"
...
# Cria arquivos
"" *> $PathLogErro
"nome_sdr;ponto_sdr;senha_sdr" *> $PathListaFinal
...
$ArqLista | ForEach-Object {
    ...
    # Se o usuário existe, edita o usuário
    if ($?) {

        # Cria a conta
        #$usuario = Set-ADUser -Identity $usuario.DistinguishedName -AccountPassword (ConvertTo-SecureString -AsPlainText $senhasdr -Force) -PassThru:$true -Confirm:$Confirm -WhatIf:$WhatIf
        Set-ADAccountPassword -Identity $usuario.DistinguishedName -Reset -NewPassword (ConvertTo-SecureString -AsPlainText $senhasdr -Force) -Confirm:$Confirm -WhatIf:$WhatIf
            if($?){"[Set-ADUser] Usuario $pontosdr editado." *>> $PathLog}
                else{"[Set-ADUser] Ocorreu um erro ao editar o usuario $pontosdr." *>> $PahLogErro}
        ...
        "[New-Mailbox] Usuário $pontosdr não existe." *>> $PahLogErro
        $UsuariosNaoEditadoss++
...
"Usuários criados: $UsuariosEditados" *>> $PathLog
"Usuários com erro na criação: $UsuariosNaoEditados" *>> $PathLog

if ($UsuariosTotal -ne ($UsuariosEditados + $UsuariosNaoEditados)) {
    "ATENÇÃO: Existe uma diferença entre a qtde. de usuários total e a qtde. de usuários editados mais os não editados." *>> $PathLog
