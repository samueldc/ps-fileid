...
$PathLog = "C:\Users\p_7029\ownCloud\_trabalho\teletrabalho\sdr\criaUsuariosTesteStressKeycloakSDR-" + (Get-Date -UFormat %Y-%m-%d-%H-%M) + ".log"
$PathLogErro = "C:\Users\p_7029\ownCloud\_trabalho\teletrabalho\sdr\criaUsuariosTesteStressKeycloakSDR-" + (Get-Date -UFormat %Y-%m-%d-%H-%M) + ".logerro"

# Caminho e nome do arquivo CSV com a lista de usuarios a ser importada
$PathLista = "C:\Users\p_7029\ownCloud\_trabalho\teletrabalho\sdr\usuariosTesteStressKeycloakSDR.csv"
...
$ArqLista | ForEach-Object {
    ...
    if (!$?) {
        ...
        # Cria a conta
        $usuario = New-ADUser -Name $pontosdr -DisplayName "Teste Stress SDR" -GivenName "Teste Stress" -Surname "SDR" -Description "Solicitação do chamado OTRS 00590927" -Title "Sistema" -Department "DIRETORIA DE INOVAÇÃO E TECNOLOGIA DA INFORMAÇÃO" -UserPrincipalName ($pontosdr + "@redecamara.camara.gov.br") -Path $OUsdr -ChangePasswordAtLogon $false -Enabled $true -AccountPassword (ConvertTo-SecureString -AsPlainText $senhasdr -Force) -PassThru:$true -Confirm:$Confirm -WhatIf:$WhatIf
